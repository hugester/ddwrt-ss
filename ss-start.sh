...
echo "server=127.0.0.1#5353" >> /tmp/dnsmasq.conf 

dnsmasq -u root -g root --conf-file=/tmp/dnsmasq.conf --cache-size=1500 --dhcp-option=252
chinadns -l /opt/etc/chinadns_iplist.txt -c /opt/etc/chinadns_chnroute.txt -p 5353 -s 114.114.144.144,127.0.0.1:1081 -m &
ss-tunnel -c /opt/etc/tunnel.json -u -L 8.8.8.8:53 -f /var/null
ss-redir -c /opt/etc/ssredir.json -f /var/null

#add the iptables table to enable the tcp redir
iptables --flush 
